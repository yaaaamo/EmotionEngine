...
# Allowed values (lowercase, inputs will be normalized)
...
def react(emotion, confidence):
    ...
    This keeps the same emotion × confidence → action mapping as in Week 1,
    while making the inputs robust to case differences (e.g. "ANGER", "High").

    Args:
        emotion: The detected emotion (string, any case)
        confidence: The confidence level of the detection (string, any case)
    ...
    if emo not in ALLOWED_EMOTIONS or conf not in ALLOWED_CONFIDENCES:
        return "ask_clarification", "Invalid input provided. Could you please clarify?"

    # Get action from mapping (core logic unchanged)
    action = REACTION_MAP[emo][conf]
